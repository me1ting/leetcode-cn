...
impl Solution {
    ...
    fn is_balanced_and_depth(node: &Option<Rc<RefCell<TreeNode>>>) -> (bool,i32){
        match node {
            Some(val)=>{
                let (left_balance,left_depth) = Solution::is_balanced_and_depth(&val.borrow().left);
                if !left_balance {
                    return (false,-1)
                }
                let (right_balance,right_depth) = Solution::is_balanced_and_depth(&val.borrow().right);
                if !right_balance {
                    return (false,-1)
                }
